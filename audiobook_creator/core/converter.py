...
class AudiobookConverter:
    ...
    def _create_m4b(self, book_dir, book_title, chapter_files):
        ...
        try:
            # Create concat file listing all chapters
            ...
            # Write metadata file with chapters
            ...
            # Use ffmpeg to combine files with efficient AAC encoding
            logger.info("Starting FFmpeg process with AAC encoding...")
            cmd = (
                f'ffmpeg -y -f concat -safe 0 -i "{concat_file}" '
                f'-i "{meta_file}" -map_metadata 1 '
                f'-c:a aac -b:a 64k '  # Use AAC encoding with 64kbps (good for speech)
                f'-af aresample=resampler=soxr '  # Use high quality resampler
            ...
            # Clean up original chapter files and temporary files
